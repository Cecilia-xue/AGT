...
def pil_loader(path):
    ...
    return torch.from_numpy(np.array(img)).permute(2, 0, 1).unsqueeze(dim=0)/255.0


def convert(cifar_dir, dataset, category_num, model, set):
    cifar_source_dir = cifar_dir + '/{}_rgb'.format(dataset)
    with open(cifar_source_dir+'/{}_list.txt'.format(set), 'r') as f:
        train_list = f.readlines()
    train_list = [line.rstrip() for line in train_list]

    cifar_hsi_dir = os.path.join(cifar_dir, '{}_phsi'.format(dataset), set)
    for i in range(category_num):
        path = os.path.join(cifar_hsi_dir, str(i))
        if not os.path.exists(path): os.makedirs(path)

    with torch.no_grad():
        with open(os.path.join(cifar_dir, '{}_phsi'.format(dataset), '{}_list.txt'.format(set)), 'w') as f:
            for sample_info in train_list:
                sample_dir, sample_label = sample_info.split()
                input = pil_loader(os.path.join(cifar_source_dir,sample_dir)).cuda()
                output = model(input)
                save_name = sample_dir.split('/')[-1].replace('jpg', 'npy')
                save_dir = os.path.join(cifar_hsi_dir, sample_label, save_name)
                np.save(save_dir, output.cpu().squeeze(dim=0).numpy())
                info = os.path.join(set, sample_label, save_name) + ' {}\n'.format(sample_label)
                f.write(info)
                print(save_dir)

if __name__ == "__main__":

    cifar_dir = '/home/disk1/data/CIFAR'
    # dataset='cifar10'
    #     # category_num=10
    dataset='cifar100'
    category_num=100
    ...
    model.load_state_dict(torch.load('/home/disk1/code/TriHiT-adapter-xxz/data/cifar_preprocess/convert_model/model.pth')['state_dict'])
    model.cuda()
    model.eval()

    convert(cifar_dir, dataset, category_num, model, 'train')
    convert(cifar_dir, dataset, category_num, model, 'test')
